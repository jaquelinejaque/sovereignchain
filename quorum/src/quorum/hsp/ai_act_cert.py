"""EU AI Act 2026-08 readiness — per-query PDF evidence record generator.

Licensed under the Apache License, Version 2.0.
You may obtain a copy of the License at http://www.apache.org/licenses/LICENSE-2.0.

HSP Commercial Restrictions:
    This module produces PDF evidence records that reference the Human
    Supervision Protocol (HSP), patent pending PCT/US26/11908. Commercial
    deployment of the record format requires a license from Sovereign Chain Ltd.
    See LICENSE-HSP at the repo root.

LEGAL NOTICE — IMPORTANT:
    The PDF artifact produced by this module is a tamper-evident
    *evidence record* (advisory technical material). It is NOT a
    conformity assessment under Regulation (EU) 2024/1689 and Sovereign
    Chain Ltd is NOT a Notified Body designated under Article 31 of that
    Regulation. Final conformity assessment remains the responsibility
    of the AI system provider (internal procedure, Annex VI) or a
    designated Notified Body (external procedure, Annex VII, e.g. for
    biometrics and certain safety components per Article 43).

    Function names that include "cert" / "certificate" are retained for
    backward compatibility with code written prior to the 0.2.4
    vocabulary clean-up; semantically they produce evidence records,
    not regulatory certificates.

WHY this module exists:
    The EU AI Act (entered into force 2024-08; high-risk provisions binding
    from 2026-08-02) requires high-risk AI inference to be (a) logged,
    ...
    query + one HSP decision into a single immutable evidence artifact (PDF
    preferred, Markdown fallback) that providers can include in their
    internal Annex VI documentation file.
    ...
    Design choices:
      * reportlab is a heavy optional dep. We import lazily so the rest of
        ...
        Markdown file with identical structure so the traceability log is
        never broken by a missing package.
      * Every record is hashed (SHA-256 over the rendered bytes) so
        downstream chain-of-custody can detect tampering without re-parsing
        the PDF.
      * The record is deterministic given (query, consensus, decision): we
...
def _render_markdown(
    *,
    ...
    lines: list[str] = []
    lines.append("# EU AI Act Readiness — Evidence Record")
    lines.append("")
    lines.append(
        "> **Advisory evidence record (not a conformity assessment).** "
        "Generated by the Quorum HSP module of Sovereign Chain Ltd. "
        "Sovereign Chain Ltd is not a Notified Body designated under Article 31 "
        "of Regulation (EU) 2024/1689. Final conformity assessment remains the "
        "responsibility of the AI system provider (internal, Annex VI) or a "
        "designated Notified Body (external, Annex VII)."
    )
    lines.append("")
    lines.append(f"- **Record version:** {CERT_VERSION}")
    lines.append(f"- **Regulation referenced:** {EU_AI_ACT_REFERENCE}")
    ...
    lines.append("---")
    lines.append(
        "Machine-generated by Sovereign Chain Ltd. via the HSP Protocol "
        f"({PATENT_REF}). Verify the signature above against HSP_PROTOCOL_KEY "
        "before relying on this record. **This is an advisory evidence record, "
        "not a conformity assessment under Regulation (EU) 2024/1689.**"
    )
    return "\n".join(lines) + "\n"
...
def _render_pdf(
    *,
    ...
    c.drawString(20 * mm, height - 25 * mm, "EU AI Act Readiness — Evidence Record")
    c.setFont("Helvetica-Oblique", 7)
    c.drawString(
        20 * mm,
        height - 30 * mm,
        "Advisory evidence record — NOT a conformity assessment under Regulation (EU) 2024/1689.",
    )
    c.setFont("Helvetica", 9)
    c.drawString(20 * mm, height - 35 * mm, f"Regulation referenced: {EU_AI_ACT_REFERENCE}")
    c.drawString(20 * mm, height - 40 * mm, f"HSP Patent Reference: {PATENT_REF}")
    c.drawString(
        20 * mm,
        height - 45 * mm,
        f"Generated (UTC): {generated_at.isoformat()}  |  Record v{CERT_VERSION}",
    )
    ...
    # Footer — mandatory legal disclaimer + signature line
    c.setFont("Helvetica-Oblique", 7)
    c.drawString(
        20 * mm,
        21 * mm,
        "Advisory evidence record only. Sovereign Chain Ltd is NOT a Notified Body under Article 31 of",
    )
    c.drawString(
        20 * mm,
        18 * mm,
        "Regulation (EU) 2024/1689. Conformity assessment remains the responsibility of the AI system provider",
    )
    c.drawString(
        20 * mm,
        15 * mm,
        "(Annex VI, internal) or a designated Notified Body (Annex VII, external) per Article 43.",
    )
    c.drawString(
        20 * mm,
        12 * mm,
        f"Machine-generated via HSP Protocol ({PATENT_REF}). Verify signature against HSP_PROTOCOL_KEY.",
    )
    c.drawString(
        20 * mm,
        9 * mm,
...
def generate_cert_pdf(
    ...
    """Generate an EU AI Act readiness PDF evidence record, falling back to Markdown.

    (Function name kept as ``generate_cert_pdf`` for backward compatibility
    with code written prior to the 0.2.4 vocabulary clean-up. Semantically
    it produces an advisory evidence record, not a regulatory certificate.)

    WHY return a dict instead of just a path: downstream (Stripe receipts,
    traceability indexer, supabase mirror) all want the SHA-256 and the
...
def _test_markdown_fallback_when_reportlab_missing(tmp_dir: Path, monkeypatch: Any) -> None:
    ...
    assert "EU AI Act Readiness — Evidence Record" in body
